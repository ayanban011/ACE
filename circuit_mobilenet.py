import os
os.environ['CUDA_VISIBLE_DEVICES'] = '6'  # Set torch cache directory
...
dataset_artpainting = datasets.ImageFolder("./OfficeHomeDataset_10072016/Art", transform=transform)
...
# -----------------------------
# MODEL (MobileNetV2 version)
# -----------------------------
class MobileNetV2_Adapter(nn.Module):
...
model_mobilenet_artpainting = MobileNetV2_Adapter(NUM_CLASSES).to(DEVICE)

# -----------------------------
# TRAIN WITH EARLY STOPPING
# -----------------------------
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(filter(lambda p: p.requires_grad, model_mobilenet_artpainting.parameters()), lr=1e-3)
...
def train_epoch():
    model_mobilenet_artpainting.train()
    total, correct = 0,0
    for x,y in train_loader_artpainting:
        x,y = x.to(DEVICE), y.to(DEVICE)
        optimizer.zero_grad()
        out = model_mobilenet_artpainting(x)
...
@torch.no_grad()
def evaluate():
    model_mobilenet_artpainting.eval()
    total, correct = 0,0
...
for e in range(EPOCHS):
    ...
    if val_acc > best_val:
        best_val = val_acc
        patience_counter = 0
        torch.save(model_mobilenet_artpainting.state_dict(), os.path.join(SAVE_DIR, "best_model_art_vgg19.pth"))
        print("✅ Model saved")
    else:
        break
...
# LOAD BEST MODEL
# -----------------------------
model_mobilenet_artpainting.load_state_dict(torch.load(os.path.join(SAVE_DIR, "best_model_art_vgg19.pth")))
model_mobilenet_artpainting.eval()
...
print("\n=== CIRCUIT EXTRACTION ===")
tracer_artpainting_mobilenet = AdapterCircuitTracer(model_mobilenet_artpainting)
circuits_artpainting_mobilenet = tracer_artpainting_mobilenet.compute_circuits(train_loader_artpainting)

# -----------------------------
# SAVE CIRCUITS
# -----------------------------
import pickle
circuit_path = os.path.join(SAVE_DIR, "circuits_vgg_art.pkl")
with open(circuit_path, "wb") as f:
    pickle.dump(circuits_artpainting_mobilenet, f)
...
# visualize one class
cls = list(circuits_artpainting_mobilenet.keys())[0]
print(cls)
print(circuits_artpainting_mobilenet[cls])
visualize_layered(circuits_artpainting_mobilenet[cls])

#import webbrowser
#webbrowser.open("circuit_layered.html")


dataset_cartoon = datasets.ImageFolder("./OfficeHomeDataset_10072016/Clipart", transform=transform)
...
model_cartoon = MobileNetV2_Adapter(NUM_CLASSES).to(DEVICE)
...
for e in range(EPOCHS):
    ...
    if val_acc > best_val:
        best_val = val_acc
        patience_counter = 0
        torch.save(model_cartoon.state_dict(), os.path.join(SAVE_DIR, "best_model_clipart_vgg.pth"))
        print("✅ Model saved")
    else:
        break
...
# LOAD BEST MODEL
# -----------------------------
model_cartoon.load_state_dict(torch.load(os.path.join(SAVE_DIR, "best_model_clipart_vgg.pth")))
model_cartoon.eval()

print("\n=== CIRCUIT EXTRACTION ===")
tracer_cartoon_mobilenet = AdapterCircuitTracer(model_cartoon)
circuits_cartoon_mobilenet = tracer_cartoon_mobilenet.compute_circuits(train_loader_cartoon)

# -----------------------------
# SAVE CIRCUITS
# -----------------------------
import pickle
circuit_path = os.path.join(SAVE_DIR, "circuits_vgg_clipart.pkl")
with open(circuit_path, "wb") as f:
    pickle.dump(circuits_cartoon_mobilenet, f)

print(f"✅ Circuits saved at {circuit_path}")

# visualize one class
cls = list(circuits_cartoon_mobilenet.keys())[0]
print(cls)
print(circuits_cartoon_mobilenet[cls])
visualize_layered(circuits_cartoon_mobilenet[cls])
...
circuit_faithfulness(model_mobilenet_artpainting, circuits_artpainting_mobilenet, test_loader_artpainting, "cuda")
...
paths = extract_top_paths(circuits_artpainting_mobilenet[0], top_k=10)
...
results = compare_circuits(circuits_artpainting_mobilenet, circuits_cartoon_mobilenet)

for cls, stats in results.items():
    print(f"Class {cls}: Jaccard {stats['jaccard']:.3f}")

dataset_photo = datasets.ImageFolder("./OfficeHomeDataset_10072016/Real_World", transform=transform)
...
model_photo = MobileNetV2_Adapter(NUM_CLASSES).to(DEVICE)
...
for e in range(EPOCHS):
    ...
    if val_acc > best_val:
        best_val = val_acc
        patience_counter = 0
        torch.save(model_photo.state_dict(), os.path.join(SAVE_DIR, "best_model_real_vgg.pth"))
        print("✅ Model saved")
    else:
        break
...
# LOAD BEST MODEL
# -----------------------------
model_photo.load_state_dict(torch.load(os.path.join(SAVE_DIR, "best_model_real_vgg.pth")))
model_photo.eval()

print("\n=== CIRCUIT EXTRACTION ===")
tracer_photo_mobilenet = AdapterCircuitTracer(model_photo)
circuits_photo_mobilenet = tracer_photo_mobilenet.compute_circuits(train_loader_photo)

# -----------------------------
# SAVE CIRCUITS
# -----------------------------
import pickle
circuit_path = os.path.join(SAVE_DIR, "circuits_vgg_real.pkl")
with open(circuit_path, "wb") as f:
    pickle.dump(circuits_photo_mobilenet, f)

print(f"✅ Circuits saved at {circuit_path}")

# visualize one class
cls = list(circuits_photo_mobilenet.keys())[0]
print(cls)
print(circuits_photo_mobilenet[cls])
visualize_layered(circuits_photo_mobilenet[cls])
