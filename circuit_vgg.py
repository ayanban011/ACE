import os
os.environ['CUDA_VISIBLE_DEVICES'] = '7'  # Set torch cache directory
...
for e in range(EPOCHS):
    ...
    if val_acc > best_val:
        best_val = val_acc
        patience_counter = 0
        torch.save(model.state_dict(), os.path.join(SAVE_DIR, "best_model_art_vgg.pth"))
        print("✅ Model saved")
    else:
        break
...
# LOAD BEST MODEL
# -----------------------------
model.load_state_dict(torch.load(os.path.join(SAVE_DIR, "best_model_art_vgg.pth")))
model.eval()
...
print("\n=== CIRCUIT EXTRACTION ===")
tracer_artpainting_vgg = AdapterCircuitTracer(model)
circuits_artpainting_vgg = tracer_artpainting_vgg.compute_circuits(train_loader)
...
with open(circuit_path, "wb") as f:
    pickle.dump(circuits_artpainting_vgg, f)
...
# visualize one class
cls = list(circuits_artpainting_vgg.keys())[0]
print(cls)
print(circuits_artpainting_vgg[cls])
visualize_layered(circuits_artpainting_vgg[cls])
...
model_cartoon = VGG19_Adapter(NUM_CLASSES).to(DEVICE)
...
print("\n=== CIRCUIT EXTRACTION ===")
tracer_cartoon_vgg = AdapterCircuitTracer(model_cartoon)
circuits_cartoon_vgg = tracer_cartoon_vgg.compute_circuits(train_loader_cartoon)
...
with open(circuit_path, "wb") as f:
    pickle.dump(circuits_cartoon_vgg, f)

print(f"✅ Circuits saved at {circuit_path}")

# visualize one class
cls = list(circuits_cartoon_vgg.keys())[0]
print(cls)
print(circuits_cartoon_vgg[cls])
visualize_layered(circuits_cartoon_vgg[cls])
...
circuit_faithfulness(model, circuits_artpainting_vgg, test_loader, "cuda")
...
paths = extract_top_paths(circuits_artpainting_vgg[0], top_k=10)
...
results = compare_circuits(circuits_artpainting_vgg, circuits_cartoon_vgg)
...
model_photo = VGG19_Adapter(NUM_CLASSES).to(DEVICE)
...
print("\n=== CIRCUIT EXTRACTION ===")
tracer_photo_vgg = AdapterCircuitTracer(model_photo)
circuits_photo_vgg = tracer_photo_vgg.compute_circuits(train_loader_photo)
...
with open(circuit_path, "wb") as f:
    pickle.dump(circuits_photo_vgg, f)

print(f"✅ Circuits saved at {circuit_path}")

# visualize one class
cls = list(circuits_photo_vgg.keys())[0]
print(cls)
print(circuits_photo_vgg[cls])
visualize_layered(circuits_photo_vgg[cls])
